...
# Execution:
	torchrun --nproc_per_node 2 --master_port 12345 imagenet.py -a mbv2_ca -b 128 --data .. \
	--dist-url 'env://' --world-size 2 --workers 10
'''
...
def main():
    ...
    model = custom_models.__dict__[args.arch](width_mult=args.width_mult)
    ...
    if not args.distributed:
        ...
        model.cuda()
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[rank], broadcast_buffers=False, find_unused_parameters=True)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, epoch):
    ...
    for i, (input_t, target) in enumerate(train_loader):
        ...
        top1.update(prec1.item(), input_t.size(0))
        top5.update(prec5.item(), input_t.size(0))
