...
class custom_att(nn.Module):
    def __init__(self, channel1, channel2, ratio=32):
        super(custom_att, self).__init__()

        relu = h_swish()
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), # 自适应平均池化，输出一维向量
            nn.Conv2d(channel1, channel1//ratio, 1, 1, 0),# 与下面注释效果一致
            # nn.Linear(inplanes, inplanes // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(channel1//ratio, channel1, 1, 1, 0),# 与下面注释效果一致
...
class MBV2_32(nn.Module):
    def __init__(self, num_classes=1000, width_mult=1.):
        super(MBV2_32, self).__init__()
