from .utils import IntermediateLayerGetter #, IntermediateLayerGetter_mvit
...
def _segm_moconv(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if output_stride == 8:
    ...
    if name == 'deeplabv3plus':
        ...
        classifier = DeepLabHead(inplanes, num_classes, aspp_dilate)
    # backbone = IntermediateLayerGetter_moconv(backbone, return_layers=return_layers)
    model = DeepLabV3(backbone, classifier)
    return model

def _segm_mvit(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if output_stride == 8:
        replace_stride_with_dilation = [False, False, True, True]
        aspp_dilate = [12, 24, 36]
    else:
    ...
    backbone =mvit.mobile_vit_small()
...
def _load_model(arch_type, backbone, num_classes, output_stride, pretrained_backbone):
    ...
    elif backbone.startswith('moconv'):
        model = _segm_moconv(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
    elif backbone.startswith('mvit'):
        model = _segm_mvit(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
...
def deeplabv3_resnet18(num_classes=21, output_stride=4, pretrained_backbone=True):
    """Constructs a DeepLabV3 model with a ResNet-50 backbone.
...
def deeplabv3_moconv(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3 model with a ResNet-50 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3', 'moconv_small', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)

def deeplabv3_mvit(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3 model with a ResNet-50 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3', 'mvit', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
