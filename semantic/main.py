...
def get_argparser():
    ...
    parser.add_argument("--gpu_id", type=str, default='0',
                        help="GPU ID")
    parser.add_argument("--weight_decay", type=float, default=5e-4,  #orginal 1e-4
...
def main():
    ...
    cur_itrs = 0
    cur_epochs = 0
    #print("zzzzzyyyyyyxxxxxxxxxxxxxx")
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
    # if opts.ckpt is  None and os.path.isfile(opts.ckpt):
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    save_ckpt('checkpoints/best_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    print("The best Mean IOU is %s \n" %best_score)
                    f = open(lf,'a')
                    a = 'Iters:  {} ; Mean IoU is {}\n'.format(cur_epochs,best_score)
                    f.write(str(a))
                    f.close
